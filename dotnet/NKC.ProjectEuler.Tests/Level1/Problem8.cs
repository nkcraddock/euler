using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace NKC.ProjectEuler.Tests.Level1
{
    [TestClass]
    public class Problem8 : ProjectEulerTestBase
    {
        const string THE_BIG_NUMBER = "";
        const int CONSECUTIVE_DIGITS = 5;
        [TestMethod]
        public void Solve()
        {
            int answer = GetMaxProductOfAnyThreeNumbersInTheBigNumber();
            Console.WriteLine(answer);
        }

        private int GetMaxProductOfAnyThreeNumbersInTheBigNumber()
        {
            int max = 0;
            int current;
            for (int i = 0; i < THE_BIG_NUMBER.Length - (CONSECUTIVE_DIGITS - 1); i++)
            {
                current = GetProductOfTheThreeNumbersAt(i);
                if (current > max)
                    max = current;
            }
            return max;
        }

        private int GetProductOfTheThreeNumbersAt(int index)
        {
            int product = GetIntAt(index);
            for (int i = 1; i < CONSECUTIVE_DIGITS; i++)
                product *= GetIntAt(index + i);
            return product;
        }

        private int GetIntAt(int ix)
        {
            return int.Parse(THE_BIG_NUMBER[ix].ToString());
        }
    }
}
